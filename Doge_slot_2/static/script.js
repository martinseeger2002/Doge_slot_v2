document.addEventListener('DOMContentLoaded', () => {
    ...
    function displaySlotLayout(zIndex) {
        ...
        slotLayoutImage.style.left = '0px';
        slotLayoutImage.style.top = '0px';
    ...
    function getReelResults() {
        return new Promise((resolve, reject) => {
            fetch('http://192.168.1.214:5000/get-reel-results')
    ...
    function updateCredits() {
        return fetch('http://192.168.1.214:5000/get-credits')
            .then(response => response.json())
            .then(data => {
    ...
    function updatebet() {
        return fetch('http://192.168.1.214:5000/get-bet')
            .then(response => response.json())
            .then(data => {
    ...
    function updateWin() {
        fetch('http://192.168.1.214:5000/get-win')
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                                ...
                                setTimeout(() => {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            }, 500); // 1000ms delay before stopping reel 5
        
                                        }, 120); // 300ms delay before displaying reel 4 results
        
                                    }, 500); // 1000ms delay before stopping reel 4
        
                                }, 110); // 250ms delay before displaying reel 3 results
        
                            }, 500); // 1000ms delay before stopping reel 3
        
                        }, 100); // 200ms delay before displaying reel 2 results
        
                    }, 500); // 1000ms delay after reel 2 results
    ...
    function onBetButtonClick() {
        ...
        fetch('http://192.168.1.214:5000/change-bet', { 
            method: 'POST',
            headers: {
    ...
    function createCashOutButton() {
        ...
        cashOutButton.style.backgroundColor = 'red';
    ...
    function onCashOutButtonClick() {
        ...
        fetch('http://192.168.1.214:5000/send-cashOut', { 
            method: 'POST',
             headers: {
                'Content-Type': 'application/json'
            }
